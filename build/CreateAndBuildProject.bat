...
cd xmaterial_instance_compiler.vs2022

cmake --build . --config Debug --target xmaterial_instance_compiler
if errorlevel 1 (
    echo Debug compilation failed.
    goto DONE:
)

cmake --build . --config Release --target xmaterial_instance_compiler
if errorlevel 1 (
